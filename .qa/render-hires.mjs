// (hi-res copy of render-pages.mjs → extract/renders-hires) Render PDF pages exactly as printed (pdf.js inside the local Chromium),
...
await page.goto(`http://localhost:${port}/tmp/render-hires.html`);
await page.waitForFunction(() => window.ready === true, null, { timeout: 60000 });
await fs.mkdir("extract/renders-hires", { recursive: true });
for (const n of pages) {
  ...
  const out = `extract/renders-hires/${edition}-p${n}.png`;
